...
import { setClickLogout, setRoleSlice, setId } from "./redux/slice/authSlice";
...
function App() {
  ...
  return (
    <>
    {(!isLogin && !register) && <Navigate to="/"/>}
    {(!isLogin && register) && <Navigate to="/register"/>}
    {isLogin ? <>
      {(role === `"NV"` || role === `"NS"` || role === `"QT"`) &&
      <Routes>
